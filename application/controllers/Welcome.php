...
class Welcome extends CI_Controller
{
    public function __construct()
    {
        ...
        if ($role_id != "") {
            if ($role_id == 2) {
                redirect('frontend/create_basic_profile');
            } else {
                redirect('Main');
            }
        }
    ...
    public function create_registration()
    {
       ...
       // ]);
        ...
        $url = "https://enlistwork.com/frontend/set_password ";
        $message = 'Please Click Here To Verify Your Account And Here Is The Verification Code' .  $url.' 
        Varification Code: '.$rand;
        $this->load->library('email', $config,$insert_id);
        $this->email->set_newline("\r\n");
        $this->email->from('shakibuli08@gmail.com'); // change it to yours
        $this->email->to($email);// change it to yours
        $this->email->subject('Enlist Work: Verification For Creatining Account.');
        ...
        if ($this->email->send()) {
            if ($insert_id) {
             ...
             redirect('frontend/set_password', 'refresh');
        } 
        else {
    ...
    public function check_login()
    {
        ...
        $email = $this->input->post('email');
        $password = md5($this->input->post('password'));
        $result = $this->Welcome_model->check_user_login($email, $password);
        if ($result) {
            $set_result = $result;
        } else {
        ...
        if ($set_result) {
            ...
            $err_mas['msg'] = "<h5 style='color: red; font-size: 15px; padding-top: 10px;'>Your user email or password invalid!!!</h5>";
    ...
    public function sent_email()
    {
      ...
      $result= $this->Main_model->insertData($data, 'forget_password');
        ...
        $url = "https://enlistwork.com/welcome/forget_password/";
