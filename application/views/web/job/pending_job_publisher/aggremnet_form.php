        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="#" method="post" id="agreement_from">
                        ...
                        <textarea class="form-control" name="description" id="exampleFormControlTextarea4" cols="30" rows="7" placeholder="Work Description"></textarea>
                        </div>
                        <?php } ?>
                        <br>
                        <div class="row">
                            <div class="col-md-6">
                                <label for="exampleFormControlTextarea4">Completion Time </label>
                                <input type="text" name="time" class="form-control" placeholder="Completion Time ">
                            </div>
                            <div class="col-md-6">
                                <label for="exampleFormControlTextarea4">Final Budget</label>
                                <input type="text" name="budget" class="form-control" placeholder="Final Budget">
                            </div>
                        </div>
                        <br>
                        <div class="col-md-12">
...
<script>
    $(document).ready(function ($id) {
        $('#agreement_from').submit(function () {
            var dataString = $('#agreement_from').serialize();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() ?>pending_job/add_agreement_from',
                data: dataString,
                success: function (result) {
                    if (result) {
                        $('#experience_msg').show();
                        $('#experience_msg .alert').delay(5000).fadeOut(1000);
                        $('#agreement_from').trigger("reset");
...
<script>
    function job_hire_bidders($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>pending_job/hire_bidder_active/' + $id,
            success: function (result) {
                if (result) {
                    return false;
                } else {
